...
class Signup(Base):
    """Signup model for family outing registrations"""
    ...
    def __repr__(self):
        return f"<Signup(id={self.id}, outing_id={self.outing_id}, contact={self.family_contact_name})>"
    ...
    def scout_count(self):
        ...
        return sum(1 for p in self.participants if not p.is_adult)

    @property
    def adult_count(self):
        """Get number of adults in this signup"""
        return sum(1 for p in self.participants if p.is_adult)
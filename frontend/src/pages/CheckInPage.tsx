...
const CheckInPage: React.FC = () => {
  ...
  const [filterType, setFilterType] = useState<
    "all" | "scouts" | "adults" | "checked-in" | "not-checked-in"
  >("all");
  ...
  const handleCheckIn = async (participantIds: string[]) => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to check in participants"
      );
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const filename = `checkin_${summary.outing_name.replace(/\s+/g, "_")}_${
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
        ...
        >
          ...
          >
            {Math.round(
              (summary.checked_in_count / summary.total_participants) * 100
            )}
            %
          </div>
      ...
      >
        ...
        >
          {/* Adults Section */}
          {filteredParticipants.filter((p) => p.member_type === "adult")
            .length > 0 && (
            <>
              <div
                className="p-3 font-bold text-sm uppercase tracking-wider"
                style={{
                  backgroundColor: "var(--bg-tertiary)",
                  ...
                  <div
                    ...
                    onClick={() =>
                      participant.is_checked_in
                        ? handleUndoCheckIn(participant.id)
          ...
          {/* Scouts Section */}
          {filteredParticipants.filter((p) => p.member_type === "scout")
            .length > 0 && (
            <>
              <div
                className="p-3 font-bold text-sm uppercase tracking-wider"
                style={{
                  ...
                  color: "var(--text-secondary)",
                  borderBottom: "1px solid var(--border-light)",
                  borderTop: filteredParticipants.some(
                    (p) => p.member_type === "adult"
                  )

...
const generateContours = (
  ...
  const getSegmentsForThreshold = (threshold: number) => {
    ...
    for (let i = 0; i < cols - 1; i++) {
      for (let j = 0; j < rows - 1; j++) {
        ...
        const right = {
          x: x + gridSize,
          y: y + gridSize * lerp(b, c, threshold),
        };
        const bottom = {
          x: x + gridSize * lerp(d, c, threshold),
          y: y + gridSize,
        };
  ...
  riverChains.forEach((chain) => {
    ...
    const isClosed =
      Math.abs(start.x - end.x) < 0.1 && Math.abs(start.y - end.y) < 0.1;

    const path = getSmoothPath(chain, isClosed);

    if (isClosed && chain.length > 10) {
      // Only treat large enough loops as lakes
      ...
      >
        ...
        {contourPaths.map((d, i) => (
          <path
            ...
            className="topo-path"
            style={{
              strokeOpacity: i % 5 === 0 ? 0.1 : 0.05,
              strokeWidth: i % 5 === 0 ? 0.5 : 0.5,
            }}
            fill="none"
          />
        ))}
        ...
        {peaks.map((peak, i) => (
          <g key={i}>
            <circle cx={peak.x} cy={peak.y} r="2.5" fill="rgba(0,0,0,0.6)" />
            <text
              x={peak.x}
              y={peak.y - 10}
              textAnchor="middle"

...
```bash
# Create backup
docker exec scouting-outing-db pg_dump -U scouttrips scout_trip_manager > backup_$(date +%Y%m%d_%H%M%S).sql

# Restore from backup
docker exec -i scouting-outing-db psql -U scouttrips scout_trip_manager < backup_20240101_120000.sql
```
...
docker exec -it scouting-outing-db psql -U scouttrips -d scout_trip_manager -c '\l'
```
